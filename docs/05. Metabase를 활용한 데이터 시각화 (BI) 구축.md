...
![시각화 예시](../img/visualization.png)

-----

## 🎯 프로젝트 최종 아키텍처 요약

1.  **Collect (Source):** `patient_treatment.csv` (가상의 병원 데이터)
2.  **Ingest (EL):**
      * **Batch:** PySpark (`SparkSubmitOperator`) -\> Postgres 적재
      * **Streaming:** Kafka Producer -\> Spark Streaming -\> Console 출력
3.  **Transform (T):** dbt (`BashOperator`) -\> SQL 기반 데이터 모델링 (`raw` -\> `stg` -\> `mart`)
4.  **Orchestrate:** Apache Airflow (DAG를 통한 작업 순서 및 의존성 관리)
5.  **Visualize:** Metabase (최종 데이터 마트 시각화)